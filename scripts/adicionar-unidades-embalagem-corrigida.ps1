...
# 1) COMPRAS_RAW passa a gravar também unidades_por_embalagem_corrigida.
...
# 3) KG: mantém a regra existente e registra a estrutura efetivamente aplicada.
Replace-LiteralOne $importar @'
      fator:
        aplicarFator
          ? qtdUnidade
...
# 4) Conversão automática: o conteúdo informado pelo Varejo Fácil é aceito como efetivo.
Replace-LiteralOne $importar @'
      fator:
        qtdUnidade,
...
# 5) Regra manual passa a exigir e devolver o conteúdo real da embalagem separado do fator matemático.
Replace-LiteralOne $importar @'
    conversaoManual &&
...
# 6) Conversão desconhecida: não inventa o conteúdo corrigido.
Replace-LiteralOne $importar @'
  return {
    qtdAnalise: qtdItens,
    fator: 1,
...
# 7) MAPA_CONVERSAO_UNIDADE ganha a coluna independente de embalagem corrigida.
Replace-LiteralOne $importar @'
    unidadeCompra:
      cabecalhos.indexOf(
        'unidade_compra'
...
# 8) Reprocessamento também atualiza unidades_por_embalagem_corrigida.
Replace-LiteralOne $importar @'
      qtdUnidade:
...
# 9) Uma regra manual só sai da fila de pendências quando ambos os conceitos estiverem definidos.
Replace-LiteralOne $importar @'
      manual &&
      manual.status ===
        'VALIDADO' &&
